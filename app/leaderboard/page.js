...
export default function LeaderboardPage() {
  ...
  const { data: leaderboard, isLoading, error, refetch } = useQuery({
    queryKey: ['leaderboard', '2025Q3'],
    queryFn: async () => {
      const response = await fetch('/api/leaderboard?cycle_id=2025Q3&limit=200');
            ...
            <CardContent>
              <div className="flex justify-center items-end gap-4">
                {filteredLeaderboard.slice(0, 3).map((rep, index) => {
                  ...
                  return (
                    <div key={rep.rep_name} className={`text-center space-y-2 ${
                      actualPosition === 0 ? 'order-2 px-2' : actualPosition === 1 ? 'order-1 px-1' : 'order-3 px-1'
                    }`}>
                      <div className="relative">
                      ...
                      <div className="space-y-1 min-h-[80px] flex flex-col justify-between">
                        <div>
                          ...
                          }`}>{rep.points_total.toLocaleString()}</p>
                        </div>
                        
                        <Badge 
                          variant="secondary" 
                          className={`${LEVEL_COLORS[rep.level]} text-xs mx-auto`}
                        >
                      ...
                      <div className={`bg-gradient-to-t rounded-t-lg mx-auto ${
                        actualPosition === 0 ? 'from-yellow-400 to-yellow-500 h-12 w-16' :
                        actualPosition === 1 ? 'from-gray-300 to-gray-400 h-8 w-14' :
          ...
          <CardContent className="p-0">
            <div className="space-y-0">
              {filteredLeaderboard.map((rep, index) => {
                const IconComponent = LEVEL_ICONS[rep.level] || Award;
                const isCurrentUser = rep.rep_name.toLowerCase().includes(
                  ...
                  >
                    ...
                    <div className="flex items-center flex-1 ml-3">
                      ...
                      <div className="ml-3 flex-1">
                        ...
                        <Badge 
                          variant="outline" 
                          className={`${LEVEL_COLORS[rep.level]} border-0 text-xs mt-1`}
                        >
                          <IconComponent className="w-3 h-3 mr-1" />
                          {rep.level}
                        </Badge>
